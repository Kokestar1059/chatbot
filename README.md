...
## ③アプリのデプロイURL
https://chatbot-api-test-kokes.vercel.app/

> **⚠️ デプロイ版は Azure API を切ってあります（echo 応答のみ）。**
> 開発中は実際に Azure OpenAI に接続して動作確認しましたが、公開URLは
> 認証なし（v1 はユーザー認証がスコープ外）のため、誰でもアクセスでき、
> そのまま公開すると第三者に API を叩かれて Azure の利用料が発生してしまいます。
> セキュリティ・コスト上の理由から、デプロイ版は **Azure の環境変数を外し、
> 返答はオウム返し（echo）だけ**になるようにしています。
>
> **実際の AI 応答を試す場合は、下の「⑦ フリー項目 → ローカルでの動かし方」**
> に従って、自分の Azure キーを `.env.local` に入れてローカルで起動してください。
